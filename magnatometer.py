...
if (not imu.IMUInit()):
    ...
    print("$IIXDR,IMU_FAILED_TO_INITIALIZE*7C")
    if (hack - t_print) > 1.0:
...
while slorb==1:
  ...
  if (hack - t_damp) > 5.0:
      ...
      if (hack - t_fail) > 1.0:
          ...
          #cs = format(reduce(operator.xor,map(ord,imu_sentence),0),'X')    rsr
          cs="0"
          if len(cs) == 1:
                cs = "0" + cs
  ...
  if imu.IMURead():
    ...
    if (hack - t_damp) > .1:
        ...
        if (hack - t_print) > 1:
            ...
            hdmcs = format(functools.reduce(operator.xor,map(ord,hdm),0),'X')
            if len(hdmcs) == 1:
                hdmcs = "0" + hdmcs
            iihdm = "$" + hdm + "*" + hdmcs
            print (iihdm)
            ...
            hdtcs = format(functools.reduce(operator.xor,map(ord,hdt),0),'X')
            if len(hdtcs) == 1:
                hdtcs = "0" + hdtcs
            iihdt = "$" + hdt + "*" + hdtcs
	    ...
	    # iixdr ahrs data
            ...
            xdrcs = format(functools.reduce(operator.xor,map(ord,xdr),0),'X')
            if len(xdrcs) == 1:
                xdrcs = "0" + xdrcs
            iixdr = "$" + xdr + "*" + xdrcs
	    ...
	    # tirot rate of turn
            ...
            rotcs = format(functools.reduce(operator.xor,map(ord,rot),0),'X')
            if len(rotcs) == 1:
                rotcs = "0" + rotcs
            tirot = "$" + rot + "*" + rotcs
